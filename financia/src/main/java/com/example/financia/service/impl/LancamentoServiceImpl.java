...
public class LancamentoServiceImpl implements LancamentoService{
	...
	public void validar(Lancamento lancamento) {
		...
		if(lancamento.getDescricao() == null || lancamento.getDescricao().trim().equals("")) {
			throw new RegraNegocioExcepion("Informe uma descrição válido.");
		}		
		
		if(lancamento.getMes() == null || lancamento.getMes() < 1 || lancamento.getMes() > 12) {
			throw new RegraNegocioExcepion("Informe um Mês válido");
		}
		
		if(lancamento.getAno() == null || lancamento.getAno().toString().length() != 4) {
			throw new RegraNegocioExcepion("Informa um Ano válido.");
		}
		
		if(lancamento.getUsuario() == null || lancamento.getUsuario().getId() == null) {
			throw new RegraNegocioExcepion("Informe o Usuário");
		}
		
		if(lancamento.getValor() == null || lancamento.getValor().compareTo(BigDecimal.ZERO) < 1) {
			throw new RegraNegocioExcepion("Informa o valor válido");
		}
		
		if(lancamento.getTipo() == null) {
			throw new RegraNegocioExcepion("Informe o valor válido");
		}
	}
